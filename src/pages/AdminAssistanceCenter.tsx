...
const textByLanguage: Record<string, Record<string, string>> = {
  es: {
    ...
    connectToReceive: 'Genera el QR de WhatsApp Web para recibir conversaciones reales en esta bandeja interna.',
    ...
    whatsappDescription: 'Conecta el número interno con un QR de WhatsApp Web. La sesión es privada y no usa Zernio.',
    ...
    connectWhatsapp: 'Generar QR de WhatsApp',
    qrScan: 'Escanea este QR desde WhatsApp › Dispositivos vinculados.',
    qrReady: 'QR listo para vincular',
    connecting: 'Iniciando WhatsApp Web…',
    whatsappMode: 'WhatsApp Web interno · sesión privada',
    disconnect: 'Desconectar sesión',
    disconnected: 'Sesión de WhatsApp desconectada.',
    ...
    aiKeyMissing: 'La IA global aún no está configurada.',
    ...
    settingsDescription: 'WhatsApp interno funciona con una sesión Web privada y QR. Zernio queda reservado exclusivamente para el Omnicanal público. La IA usa la configuración global de Super Admin.',
    channelProvider: 'Conector de canales internos',
    aiProvider: 'Proveedor de inteligencia',
    globalAiDescription: 'Este agente usa la IA global de Super Admin. La clave no se duplica ni se guarda aquí.',
    openGlobalSettings: 'Abrir configuración AI global',
    apiUrl: 'Conector',
    apiKey: 'Sesión',
    keepExisting: 'La sesión se conserva en el servidor.',
    ...
    ticketsDescription: 'Aquí aparecen los mismos tickets reales que en el soporte de Super Admin.',
    noTickets: 'No hay tickets registrados.',
  ...
  it: {
    eyebrow: 'OPERAZIONE INTERNA · SUPER ADMIN', title: 'Centro Assistenza AI', subtitle: 'Gestisci i clienti dai tuoi canali aziendali, con contesto sicuro e passaggio al team umano.', boundary: 'Modulo interno. Non gestisce la rivendita di Omnicanal + AI.', resale: 'Vai a Omnicanal · Rivendita', refresh: 'Aggiorna', inbox: 'Posta unificata', tickets: 'Ticket', customers: 'Clienti identificati', team: 'Team', channels: 'Canali interni', agent: 'Agente AI', knowledge: 'Base di conoscenza', settings: 'Configurazione', connectedChannels: 'Canali connessi', conversations: 'Conversazioni aperte', openTickets: 'Ticket pendenti', knowledgeItems: 'Articoli attivi', notConfigured: 'Configurazione necessaria', configured: 'Configurato', internalOnly: 'Configurazione indipendente', inboxTitle: 'Posta aziendale', inboxDescription: 'Conversazioni ricevute nei tuoi canali interni. L’identità viene verificata prima dei dati privati.', searchConversation: 'Cerca contatto o messaggio…', noConversations: 'Non ci sono ancora conversazioni interne.', connectToReceive: 'Genera il QR di WhatsApp Web per ricevere conversazioni reali in questa casella interna.', selectConversation: 'Seleziona una conversazione', conversationHint: 'Puoi rispondere, verificare il cliente o trasferire il caso a un ticket.', internalAgent: 'Consulta l’agente interno', internalAgentHint: 'Chiedi di spedizioni, ticket, fatturazione, marketplace o integrazioni.', askAgent: 'Scrivi una richiesta per il team…', send: 'Invia', identify: 'Verifica cliente', identifierPlaceholder: 'Email registrata o codice DD-…', handoff: 'Passa a un operatore', handoffHint: 'È necessario verificare il cliente per creare un ticket collegato.', verified: 'Cliente verificato', unverified: 'Identità da verificare', ticketCreated: 'Ticket creato', noMessages: 'Nessun messaggio.', channelsTitle: 'Canali aziendali', channelsDescription: 'Questi canali appartengono a DoorDrop e sono separati dagli account dei clienti.', whatsapp: 'WhatsApp aziendale', whatsappDescription: 'Collega il numero interno con un QR di WhatsApp Web. La sessione è privata e non usa Zernio.', instagram: 'Instagram aziendale', facebook: 'Facebook aziendale', telegram: 'Telegram aziendale', email: 'Email aziendale', webChat: 'Web Chat aziendale', readyLater: 'Integrazione preparata separatamente', connectWhatsapp: 'Genera QR di WhatsApp', qrScan: 'Scansiona questo QR da WhatsApp › Dispositivi collegati.', qrReady: 'QR pronto per il collegamento', connecting: 'Avvio di WhatsApp Web…', whatsappMode: 'WhatsApp Web interno · sessione privata', disconnect: 'Disconnetti sessione', disconnected: 'Sessione WhatsApp disconnessa.', noChannel: 'Non connesso', agentTitle: 'Agente AI interno', agentDescription: 'Consulta i dati autorizzati per assistere il team. Non condivide la configurazione della rivendita.', aiKeyMissing: 'La configurazione IA globale non è ancora pronta.', aiReady: 'Agente pronto', model: 'Modello', humanHandoff: 'Passaggio umano', humanHandoffDescription: 'Le richieste private richiedono email o codice cliente; i casi complessi diventano ticket.', settingsTitle: 'Configurazione isolata', settingsDescription: 'WhatsApp interno usa una sessione Web privata e QR. Zernio resta riservato esclusivamente a Omnicanal pubblico. L’IA usa la configurazione globale del Super Admin.', channelProvider: 'Connettore canali interni', aiProvider: 'Provider IA', globalAiDescription: 'Questo agente usa l’IA globale del Super Admin. La chiave non viene duplicata né salvata qui.', openGlobalSettings: 'Apri configurazione IA globale', apiUrl: 'Connettore', apiKey: 'Sessione', keepExisting: 'La sessione viene conservata sul server.', modelLabel: 'Modello interno', enabled: 'Agente attivo', defaultLanguage: 'Lingua predefinita', save: 'Salva configurazione', saved: 'Configurazione salvata.', ticketsTitle: 'Ticket del Centro Assistenza', ticketsDescription: 'Qui compaiono gli stessi ticket reali del supporto del Super Admin.', noTickets: 'Nessun ticket registrato.', openSupport: 'Apri supporto completo', customersTitle: 'Clienti identificati', customersDescription: 'Sono mostrati solo i contatti verificati.', noCustomers: 'Nessun cliente identificato.', teamTitle: 'Team assistenza', teamDescription: 'Il Centro usa il team reale di supporto configurato nel Super Admin.', manageTeam: 'Gestisci team', noTeam: 'Nessun membro configurato.', knowledgeTitle: 'Base di conoscenza interna', knowledgeDescription: 'Informazioni operative che l’agente puede usare.', articleTitle: 'Titolo articolo', articleContent: 'Contenuto operativo', addArticle: 'Aggiungi articolo', noKnowledge: 'Nessun articolo attivo.', loading: 'Caricamento…', error: 'Impossibile caricare il Centro Assistenza.', retry: 'Riprova', unknown: 'Sconosciuto', date: 'Data', status: 'Stato', contact: 'Contatto', subject: 'Oggetto', language: 'Lingua'
  },
  en: {
    eyebrow: 'INTERNAL OPERATION · SUPER ADMIN', title: 'AI Assistance Center', subtitle: 'Support customers from your corporate channels with a secure context and human handoff.', boundary: 'Internal module. It does not manage Omnichannel + AI resale.', resale: 'Go to Omnichannel · Resale', refresh: 'Refresh', inbox: 'Inbox', tickets: 'Tickets', customers: 'Identified customers', team: 'Team', channels: 'Internal channels', agent: 'AI agent', knowledge: 'Knowledge base', settings: 'Settings', connectedChannels: 'Connected channels', conversations: 'Open conversations', openTickets: 'Pending tickets', knowledgeItems: 'Active articles', notConfigured: 'Needs configuration', configured: 'Configured', internalOnly: 'Independent configuration', inboxTitle: 'Corporate inbox', inboxDescription: 'Conversations received on your internal channels. Identity is verified before private data is shown.', searchConversation: 'Search contact or message…', noConversations: 'There are no internal conversations yet.', connectToReceive: 'Generate the WhatsApp Web QR to receive real conversations in this internal inbox.', selectConversation: 'Select a conversation', conversationHint: 'You can reply, verify the customer or hand the case to a ticket.', internalAgent: 'Ask the internal agent', internalAgentHint: 'Ask about shipments, tickets, billing, marketplace or integrations using system context only.', askAgent: 'Write a request for the team…', send: 'Send', identify: 'Verify customer', identifierPlaceholder: 'Registered email or DD-… code', handoff: 'Hand off to human', handoffHint: 'Customer verification is required to create a linked ticket.', verified: 'Customer verified', unverified: 'Identity pending', ticketCreated: 'Ticket created', noMessages: 'No messages yet.', channelsTitle: 'Corporate channels', channelsDescription: 'These channels belong to DoorDrop and are separate from customer resale accounts.', whatsapp: 'Corporate WhatsApp', whatsappDescription: 'Connect the internal number with a WhatsApp Web QR. The session is private and does not use Zernio.', instagram: 'Corporate Instagram', facebook: 'Corporate Facebook', telegram: 'Corporate Telegram', email: 'Corporate email', webChat: 'Corporate Web Chat', readyLater: 'Separate integration prepared', connectWhatsapp: 'Generate WhatsApp QR', qrScan: 'Scan this QR from WhatsApp › Linked devices.', qrReady: 'QR ready to link', connecting: 'Starting WhatsApp Web…', whatsappMode: 'Internal WhatsApp Web · private session', disconnect: 'Disconnect session', disconnected: 'WhatsApp session disconnected.', noChannel: 'Not connected', agentTitle: 'Internal AI agent', agentDescription: 'The agent uses authorized ecosystem data to assist the team. It does not share resale configuration.', aiKeyMissing: 'The global AI configuration is not ready yet.', aiReady: 'Agent ready', model: 'Model', humanHandoff: 'Human handoff', humanHandoffDescription: 'Private requests ask for an email or customer code; complex cases become linked tickets.', settingsTitle: 'Isolated configuration', settingsDescription: 'Internal WhatsApp uses a private Web session and QR. Zernio remains exclusively for public Omnichannel. AI uses the global Super Admin configuration.', channelProvider: 'Internal channel connector', aiProvider: 'AI provider', globalAiDescription: 'This agent uses the global Super Admin AI configuration. The key is not duplicated or stored here.', openGlobalSettings: 'Open global AI settings', apiUrl: 'Connector', apiKey: 'Session', keepExisting: 'The session is kept on the server.', modelLabel: 'Internal model', enabled: 'Agent enabled', defaultLanguage: 'Default language', save: 'Save configuration', saved: 'Configuration saved.', ticketsTitle: 'Assistance Center tickets', ticketsDescription: 'This shows the same real tickets as Super Admin support.', noTickets: 'No tickets registered.', openSupport: 'Open full support', customersTitle: 'Identified customers', customersDescription: 'Only contacts verified by email or DoorDrop code appear here.', noCustomers: 'No identified customers yet.', teamTitle: 'Assistance team', teamDescription: 'The Center uses the real support team configured in Super Admin.', manageTeam: 'Manage team', noTeam: 'No support members configured.', knowledgeTitle: 'Internal knowledge base', knowledgeDescription: 'Operational information the agent can use for accurate answers.', articleTitle: 'Article title', articleContent: 'Operational content', addArticle: 'Add article', noKnowledge: 'No active articles.', loading: 'Loading…', error: 'The Assistance Center could not be loaded.', retry: 'Retry', unknown: 'Unknown', date: 'Date', status: 'Status', contact: 'Contact', subject: 'Subject', language: 'Language'
  },
  fr: {
    eyebrow: 'OPÉRATION INTERNE · SUPER ADMIN', title: 'Centre d’assistance IA', subtitle: 'Accompagnez les clients depuis vos canaux d’entreprise avec un contexte sécurisé et un relais humain.', boundary: 'Module interne. Il ne gère pas la revente Omnicanal + AI.', resale: 'Aller à Omnicanal · Revente', refresh: 'Actualiser', inbox: 'Boîte de réception', tickets: 'Tickets', customers: 'Clients identifiés', team: 'Équipe', channels: 'Canaux internes', agent: 'Agent IA', knowledge: 'Base de connaissances', settings: 'Configuration', connectedChannels: 'Canaux connectés', conversations: 'Conversations ouvertes', openTickets: 'Tickets en attente', knowledgeItems: 'Articles actifs', notConfigured: 'Configuration requise', configured: 'Configuré', internalOnly: 'Configuration indépendante', inboxTitle: 'Boîte de réception', inboxDescription: 'Conversations reçues sur vos canaux internes. L’identité est vérifiée avant les données privées.', searchConversation: 'Rechercher un contact ou message…', noConversations: 'Aucune conversation interne pour le moment.', connectToReceive: 'Générez le QR de WhatsApp Web pour recevoir de vraies conversations dans cette boîte interne.', selectConversation: 'Sélectionnez une conversation', conversationHint: 'Répondez, vérifiez le client ou transmettez le cas à un ticket.', internalAgent: 'Interroger l’agent interne', internalAgentHint: 'Demandez des informations sur les envois, tickets, factures, marketplace ou intégrations.', askAgent: 'Écrivez une demande pour l’équipe…', send: 'Envoyer', identify: 'Vérifier le client', identifierPlaceholder: 'Email enregistré ou code DD-…', handoff: 'Transmettre à un humain', handoffHint: 'La vérification du client est requise pour créer un ticket lié.', verified: 'Client vérifié', unverified: 'Identité à vérifier', ticketCreated: 'Ticket créé', noMessages: 'Aucun message.', channelsTitle: 'Canaux d’entreprise', channelsDescription: 'Ces canaux appartiennent à DoorDrop et sont séparés des comptes de revente clients.', whatsapp: 'WhatsApp d’entreprise', whatsappDescription: 'Connectez le numéro interne avec un QR WhatsApp Web. La session est privée et n’utilise pas Zernio.', instagram: 'Instagram d’entreprise', facebook: 'Facebook d’entreprise', telegram: 'Telegram d’entreprise', email: 'Email d’entreprise', webChat: 'Web Chat d’entreprise', readyLater: 'Intégration séparée préparée', connectWhatsapp: 'Générer le QR WhatsApp', qrScan: 'Scannez ce QR depuis WhatsApp › Appareils connectés.', qrReady: 'QR prêt à être lié', connecting: 'Démarrage de WhatsApp Web…', whatsappMode: 'WhatsApp Web interne · session privée', disconnect: 'Déconnecter la session', disconnected: 'Session WhatsApp déconnectée.', noChannel: 'Non connecté', agentTitle: 'Agent IA interne', agentDescription: 'L’agent utilise les données autorisées de l’écosystème pour aider l’équipe, sans partager la configuration de revente.', aiKeyMissing: 'La configuration IA globale n’est pas encore prête.', aiReady: 'Agent prêt', model: 'Modèle', humanHandoff: 'Relais humain', humanHandoffDescription: 'Les demandes privées exigent un email ou un code client; les cas complexes deviennent des tickets.', settingsTitle: 'Configuration isolée', settingsDescription: 'WhatsApp interne utilise une session Web privée et un QR. Zernio reste réservé à Omnicanal public. L’IA utilise la configuration globale du Super Admin.', channelProvider: 'Connecteur de canaux internes', aiProvider: 'Fournisseur IA', globalAiDescription: 'Cet agent utilise la configuration IA globale du Super Admin. La clé n’est ni dupliquée ni enregistrée ici.', openGlobalSettings: 'Ouvrir la configuration IA globale', apiUrl: 'Connecteur', apiKey: 'Session', keepExisting: 'La session est conservée sur le serveur.', modelLabel: 'Modèle interne', enabled: 'Agent actif', defaultLanguage: 'Langue par défaut', save: 'Enregistrer la configuration', saved: 'Configuration enregistrée.', ticketsTitle: 'Tickets du Centre d’assistance', ticketsDescription: 'Cette vue affiche les mêmes tickets réels que le support du Super Admin.', noTickets: 'Aucun ticket enregistré.', openSupport: 'Ouvrir le support complet', customersTitle: 'Clients identifiés', customersDescription: 'Seuls les contacts vérifiés par email ou code DoorDrop apparaissent.', noCustomers: 'Aucun client identifié.', teamTitle: 'Équipe d’assistance', teamDescription: 'Le Centre utilise l’équipe réelle configurée dans le Super Admin.', manageTeam: 'Gérer l’équipe', noTeam: 'Aucun membre configuré.', knowledgeTitle: 'Base de connaissances interne', knowledgeDescription: 'Informations opérationnelles utilisables par l’agent.', articleTitle: 'Titre de l’article', articleContent: 'Contenu opérationnel', addArticle: 'Ajouter un article', noKnowledge: 'Aucun article actif.', loading: 'Chargement…', error: 'Impossible de charger le Centre d’assistance.', retry: 'Réessayer', unknown: 'Inconnu', date: 'Date', status: 'Statut', contact: 'Contact', subject: 'Objet', language: 'Langue'
  }
};

textByLanguage.it.knowledgeDescription = 'Informazioni operative che l’agente può usare.';
...
const statusClass = (status: string) => {
  ...
  if (['connected', 'ready', 'active', 'online', 'open', 'verified'].includes(value)) return 'bg-emerald-50 text-emerald-700 border-emerald-200';
  if (['needs_human', 'pending', 'starting', 'authenticated', 'qr', 'unverified'].includes(value)) return 'bg-amber-50 text-amber-700 border-amber-200';
  if (['closed', 'failed', 'inactive', 'disconnected', 'auth_failure', 'error'].includes(value)) return 'bg-slate-100 text-slate-600 border-slate-200';
...
export default function AdminAssistanceCenter() {
  ...
  const loadAll = async () => {
    ...
    try {
      ...
      setOverview(overviewRes.overview || null); setSettings(settingsRes.settings || null); setSettingsForm({ ...(settingsRes.settings || {}), ai_api_key: '' });
      setChannels(channelsRes.channels || []); setWhatsappStatus(channelsRes.whatsapp || null); setConversations(conversationsRes.conversations || []); setTickets(ticketsRes.tickets || []); setCustomers(customersRes.customers || []); setTeam(teamRes.team || []); setKnowledge(knowledgeRes.knowledge || []);
  ...
  useEffect(() => {
    if (tab !== 'channels' || !['starting', 'qr', 'authenticated'].includes(String(whatsappStatus?.status || ''))) return;
    const timer = window.setInterval(() => {
      void api.getAdminAssistanceWhatsappStatus().then((response) => {
        setWhatsappStatus(response.whatsapp || null);
        if (response.channel) setChannels((current) => [response.channel, ...current.filter((item) => item.id !== response.channel.id || item.platform !== 'whatsapp')]);
      }).catch(() => undefined);
    }, 1500);
    return () => window.clearInterval(timer);
  }, [tab, whatsappStatus?.status]);
  ...
  const connectWhatsapp = async () => {
    ...
    try { const response = await api.connectAdminAssistanceChannel('whatsapp', language); setWhatsappStatus(response.whatsapp || null); setNotice(c.connectWhatsapp); await loadAll(); } catch (e: any) { setError(e?.message || c.error); } finally { setBusy(''); }
  };

  const disconnectWhatsapp = async () => {
    if (busy) return;
    setBusy('disconnect'); setError('');
    try { const response = await api.logoutAdminAssistanceWhatsapp(); setWhatsappStatus(response.whatsapp || null); setNotice(c.disconnected); await loadAll(); } catch (e: any) { setError(e?.message || c.error); } finally { setBusy(''); }
  };

  const saveSettings = async () => {
    setBusy('settings'); setError(''); setNotice('');
    try { await api.saveAdminAssistanceSettings({ ai_enabled: settingsForm.ai_enabled, default_language: settingsForm.default_language }); setNotice(c.saved); const response = await api.getAdminAssistanceSettings(); setSettings(response.settings || null); setSettingsForm({ ...(response.settings || {}), ai_api_key: '' }); await loadAll(); } catch (e: any) { setError(e?.message || c.error); } finally { setBusy(''); }
  };
  ...
  const renderChannels = () => {
    const cards = [
      { platform: 'whatsapp', title: c.whatsapp, description: c.whatsappDescription, icon: Phone },
      { platform: 'instagram', title: c.instagram, description: c.readyLater, icon: MessageCircle },
      { platform: 'facebook', title: c.facebook, description: c.readyLater, icon: MessageCircle },
      { platform: 'telegram', title: c.telegram, description: c.readyLater, icon: MessageCircle },
      { platform: 'email', title: c.email, description: c.readyLater, icon: MessagesSquare },
      { platform: 'web_chat', title: c.webChat, description: c.readyLater, icon: MessageCircle }
    ];
    const whatsapp = whatsappStatus || {};
    const connected = channels.find((item) => item.platform === 'whatsapp');
    return <div className="space-y-5">
      <div><h2 className="text-2xl font-black text-slate-900">{c.channelsTitle}</h2><p className="mt-1 max-w-3xl text-sm text-slate-500">{c.channelsDescription}</p></div>
      <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
        {cards.map((card) => {
          const channel = card.platform === 'whatsapp' ? (connected || { status: whatsapp.status || 'pending' }) : channels.find((item) => item.platform === card.platform);
          const Icon = card.icon;
          return <div key={card.platform} className={`rounded-2xl border bg-white p-5 shadow-sm ${card.platform === 'whatsapp' ? 'border-blue-200 ring-1 ring-blue-50' : 'border-slate-200'}`}>
            <div className="flex items-start justify-between"><div className="rounded-xl bg-blue-50 p-3 text-blue-600"><Icon className="h-5 w-5" /></div><span className={`rounded-full border px-2 py-1 text-[10px] font-bold ${statusClass(channel?.status || 'pending')}`}>{channel ? (channel.status === 'connected' && whatsapp.status === 'ready' ? 'ready' : channel.status) : c.noChannel}</span></div>
            <h3 className="mt-4 font-black text-slate-900">{card.title}</h3><p className="mt-1 min-h-10 text-sm leading-5 text-slate-500">{card.description}</p>
            {card.platform === 'whatsapp' && <>
              {whatsapp.status === 'qr' && whatsapp.qrDataUrl && <div className="mt-4 rounded-2xl border border-blue-100 bg-blue-50/50 p-3 text-center"><img src={whatsapp.qrDataUrl} alt={c.qrReady} className="mx-auto h-56 w-56 rounded-xl bg-white p-2" /><p className="mt-3 text-xs font-bold text-blue-900">{c.qrScan}</p></div>}
              {['starting', 'authenticated'].includes(String(whatsapp.status)) && <div className="mt-4 flex items-center gap-2 rounded-xl bg-slate-50 p-3 text-xs font-bold text-slate-600"><Loader2 className="h-4 w-4 animate-spin text-blue-600" />{c.connecting}</div>}
              {whatsapp.status === 'ready' && <div className="mt-4 rounded-xl border border-emerald-100 bg-emerald-50 p-3 text-xs font-bold text-emerald-800">{c.whatsappMode}{whatsapp.phoneNumber ? ` · ${whatsapp.phoneNumber}` : ''}</div>}
              {whatsapp.lastError && <p className="mt-3 text-xs font-semibold text-red-600">{whatsapp.lastError}</p>}
              <div className="mt-5 flex gap-2"><button onClick={() => void connectWhatsapp()} disabled={busy === 'connect' || ['starting', 'authenticated'].includes(String(whatsapp.status))} className="inline-flex flex-1 items-center justify-center gap-2 rounded-xl bg-blue-600 px-4 py-3 text-sm font-bold text-white disabled:opacity-50">{busy === 'connect' ? <Loader2 className="h-4 w-4 animate-spin" /> : <QrCode className="h-4 w-4" />}{c.connectWhatsapp}</button>{whatsapp.status === 'ready' && <button onClick={() => void disconnectWhatsapp()} disabled={busy === 'disconnect'} className="rounded-xl border border-red-200 px-3 py-3 text-xs font-bold text-red-700 disabled:opacity-50">{c.disconnect}</button>}</div>
            </>}
            {card.platform !== 'whatsapp' && <div className="mt-5 rounded-xl bg-slate-50 px-4 py-3 text-center text-xs font-bold text-slate-500">{c.readyLater}</div>}
          </div>;
        })}
      </div>
      <div className="flex flex-wrap items-center gap-3 rounded-2xl border border-blue-100 bg-blue-50/60 p-5 shadow-sm"><QrCode className="h-6 w-6 shrink-0 text-blue-600" /><div><p className="font-black text-slate-900">{c.whatsappMode}</p><p className="mt-1 text-xs text-slate-600">{c.qrScan}</p></div></div>
    </div>;
  };
  ...
  const renderSettings = () => (
    <div className="max-w-4xl space-y-5">
      <div>
        <h2 className="text-2xl font-black text-slate-900">{c.settingsTitle}</h2>
        <p className="mt-1 text-sm text-slate-500">{c.settingsDescription}</p>
      </div>
      <div className="grid gap-5 md:grid-cols-2">
        <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
          <div className="flex items-center gap-3">
            <div className="rounded-xl bg-blue-50 p-3 text-blue-600"><QrCode className="h-5 w-5" /></div>
            <div><h3 className="font-black text-slate-900">{c.channelProvider}</h3><p className="text-xs text-slate-500">{c.internalOnly}</p></div>
          </div>
          <div className="mt-5 rounded-xl border border-blue-100 bg-blue-50/60 p-4 text-sm leading-6 text-blue-900">{c.settingsDescription}</div>
          <div className="mt-4 flex items-center justify-between rounded-xl border border-slate-100 bg-slate-50 px-4 py-3 text-xs"><span className="font-bold text-slate-600">{c.apiUrl}</span><span className="font-black text-slate-900">WhatsApp Web + QR</span></div>
          <div className="mt-2 flex items-center justify-between rounded-xl border border-slate-100 bg-slate-50 px-4 py-3 text-xs"><span className="font-bold text-slate-600">{c.apiKey}</span><span className="font-black text-emerald-700">{whatsappStatus?.sessionConfigured ? c.configured : c.qrReady}</span></div>
        </div>
        <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
          <div className="flex items-center gap-3">
            <div className="rounded-xl bg-violet-50 p-3 text-violet-600"><KeyRound className="h-5 w-5" /></div>
            <div><h3 className="font-black text-slate-900">{c.aiProvider}</h3><p className="text-xs text-slate-500">{c.globalAiDescription}</p></div>
          </div>
          <div className="mt-5 rounded-xl border border-violet-100 bg-violet-50/60 p-4">
            <div className="flex items-start gap-3">
              {settings?.ai_api_configured && settings?.ai_global_enabled ? <CheckCircle2 className="mt-0.5 h-5 w-5 shrink-0 text-emerald-600" /> : <AlertTriangle className="mt-0.5 h-5 w-5 shrink-0 text-amber-600" />}
              <div>
                <p className="text-sm font-black text-slate-900">{settings?.ai_api_configured && settings?.ai_global_enabled ? c.configured : c.aiKeyMissing}</p>
                <p className="mt-1 text-xs leading-5 text-slate-600">{c.globalAiDescription}</p>
              </div>
            </div>
            <div className="mt-4 grid gap-2 text-xs text-slate-600">
              <p><span className="font-bold">{c.apiUrl}:</span> {settings?.ai_api_url || '—'}</p>
              <p><span className="font-bold">{c.model}:</span> {settings?.ai_model || '—'} · <span className="uppercase">{settings?.ai_provider || '—'}</span></p>
            </div>
            <Link to="/admin/settings" className="mt-4 inline-flex items-center gap-2 rounded-lg border border-violet-200 bg-white px-3 py-2 text-xs font-bold text-violet-800 hover:bg-violet-50"><ExternalLink className="h-4 w-4" />{c.openGlobalSettings}</Link>
          </div>
          <label className="mt-4 flex items-center gap-3 text-sm font-bold text-slate-700">
            <input type="checkbox" checked={settingsForm.ai_enabled !== false} onChange={(event) => setSettingsForm({ ...settingsForm, ai_enabled: event.target.checked })} className="h-4 w-4 accent-blue-600" />{c.enabled}
          </label>
        </div>
      </div>
      <div className="rounded-2xl border border-blue-100 bg-blue-50/60 p-5">
        <div className="flex items-start gap-3"><ShieldCheck className="mt-0.5 h-5 w-5 shrink-0 text-blue-600" /><p className="text-sm leading-6 text-blue-900">{c.settingsDescription}</p></div>
      </div>
      <button onClick={() => void saveSettings()} disabled={busy === 'settings'} className="inline-flex items-center gap-2 rounded-xl bg-blue-600 px-5 py-3 text-sm font-bold text-white disabled:opacity-50"><Save className="h-4 w-4" />{c.save}</button>
    </div>
  );
