...
const textByLanguage: Record<string, Record<string, string>> = {
  es: {
    ...
    settingsDescription: 'La clave de Zernio y el secreto del webhook son exclusivos de los canales internos. La IA usa la configuración global de Super Admin.',
      ...
      <div className="grid gap-5 md:grid-cols-2">
        <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
          <div className="flex items-center gap-3">
            <div className="rounded-xl bg-blue-50 p-3 text-blue-600"><Webhook className="h-5 w-5" /></div>
            <div><h3 className="font-black text-slate-900">{c.channelProvider}</h3><p className="text-xs text-slate-500">{c.internalOnly}</p></div>
          </div>
          <label className="mt-5 block text-xs font-bold uppercase tracking-wide text-slate-500">{c.apiUrl}
            <input value={settingsForm.zernio_api_url || ''} onChange={(event) => setSettingsForm({ ...settingsForm, zernio_api_url: event.target.value })} className="mt-2 w-full rounded-xl border border-slate-200 px-3 py-3 text-sm outline-none focus:border-blue-400" />
          </label>
          <label className="mt-4 block text-xs font-bold uppercase tracking-wide text-slate-500">{c.apiKey}
            <input type="password" autoComplete="new-password" value={settingsForm.zernio_api_key || ''} onChange={(event) => setSettingsForm({ ...settingsForm, zernio_api_key: event.target.value })} placeholder={settings?.zernio_api_configured ? '••••••••  ' + c.keepExisting : c.channelKeyMissing} className="mt-2 w-full rounded-xl border border-slate-200 px-3 py-3 text-sm outline-none focus:border-blue-400" />
          </label>
          <label className="mt-4 block text-xs font-bold uppercase tracking-wide text-slate-500">{c.webhookSecret}
            <input type="password" autoComplete="new-password" value={settingsForm.zernio_webhook_secret || ''} onChange={(event) => setSettingsForm({ ...settingsForm, zernio_webhook_secret: event.target.value })} placeholder={settings?.zernio_webhook_configured ? '••••••••  ' + c.keepExisting : 'Se genera al activar el webhook'} className="mt-2 w-full rounded-xl border border-slate-200 px-3 py-3 text-sm outline-none focus:border-blue-400" />
          </label>
        </div>
        <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
          <div className="flex items-center gap-3">
