                          ...
                          <div className="flex items-center gap-2">
                            ...
                            {['paid', 'preparing'].includes(order.status) && !order.package_confirmed_at && (
                              <button onClick={async () => {
                                ...
                                try { const result = await api.confirmMarketplaceOrderPackage(order.id, { weightKg, lengthCm, widthCm, heightCm }); alert(result?.message || 'Medidas guardadas. DoorDrop prepara el env�o cuando haya saldo.'); await loadSellerOrders(); }
