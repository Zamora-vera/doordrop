...
export const SHIPPING_TERMS_VERSION = '1.0';
export type ShippingTermsLanguage = 'es' | 'it' | 'en';

export function resolveShippingTermsLanguage(language?: string): ShippingTermsLanguage {
  const normalized = String(language || '').toLowerCase().slice(0, 2);
  return normalized === 'it' || normalized === 'en' || normalized === 'es' ? normalized : 'en';
}

const labels: Record<ShippingTermsLanguage, {
  title: string;
  intro: string;
  back: string;
  fallback: string;
  ...
  es: {
    title: 'Términos y condiciones de DoorDrop Envíos',
    intro: 'Consulta la versión vigente de las condiciones aplicables a los servicios de envíos.',
    back: 'Volver a DoorDrop',
  ...
  it: {
    title: 'Termini e condizioni di DoorDrop Spedizioni',
    intro: 'Consulta la versione vigente delle condizioni applicabili ai servizi di spedizione.',
    back: 'Torna a DoorDrop',
  ...
  en: {
    title: 'DoorDrop Shipping Terms and Conditions',
    intro: 'Read the current terms that apply to DoorDrop shipping services.',
    back: 'Back to DoorDrop',
...
export function ShippingTermsDocument({
  language,
  compact = false
}: {
  language?: string;
  ...
  const legalLanguage = resolveShippingTermsLanguage(requestedLanguage);
  ...
  useEffect(() => {
    ...
    setMarkdown('');
    setError(false);
    fetch('/legal/shipping-terms/' + legalLanguage + '.md', { cache: 'no-store' })
      .then(response => {
        ...
        {!compact && (
          <div className="mb-6 flex items-center justify-between gap-4">
            ...
            <span className="inline-flex items-center gap-2 text-xs font-bold text-slate-500">
              <FileText className="w-4 h-4" /> Version {SHIPPING_TERMS_VERSION}
            </span>
          </div>
        )}
...
export default ShippingTermsDocument;
