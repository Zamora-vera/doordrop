...
export type MarketplaceTermsLanguage = 'es' | 'it' | 'en';

export function resolveMarketplaceTermsLanguage(language?: string): MarketplaceTermsLanguage {
  const normalized = String(language || '').toLowerCase().slice(0, 2);
  return normalized === 'it' || normalized === 'en' || normalized === 'es' ? normalized : 'en';
}

const labels: Record<MarketplaceTermsLanguage, {
  title: string;
  intro: string;
  back: string;
  fallback: string;
  ...
  es: {
    title: 'Términos y condiciones del Marketplace DoorDrop',
    intro: 'Consulta la versión vigente de las condiciones aplicables al Marketplace.',
  ...
  it: {
    title: 'Termini e condizioni del Marketplace DoorDrop',
    intro: 'Consulta la versión vigente de las condiciones aplicables al Marketplace.',
    back: 'Torna al Marketplace',
  ...
  en: {
    title: 'DoorDrop Marketplace Terms and Conditions',
    intro: 'Read the current terms that apply to the Marketplace.',
...
export function MarketplaceTermsDocument({
  language,
  compact = false
}: {
  language?: string;
  ...
  const legalLanguage = resolveMarketplaceTermsLanguage(requestedLanguage);
  ...
  useEffect(() => {
    ...
    setMarkdown('');
    setError(false);
    fetch('/legal/marketplace-terms/' + legalLanguage + '.md', { cache: 'no-store' })
      .then(response => {
      ...
      <div className={compact ? 'w-full' : 'max-w-4xl mx-auto'}>
        {!compact && (
          <div className="mb-6 flex items-center justify-between gap-4">
            <Link to="/marketplace" className="inline-flex items-center gap-2 text-sm font-bold text-blue-600 hover:text-blue-700">
              <ArrowLeft className="w-4 h-4" /> {copy.back}
            </Link>
            <span className="inline-flex items-center gap-2 text-xs font-bold text-slate-500">
              <FileText className="w-4 h-4" /> Version {MARKETPLACE_TERMS_VERSION}
            </span>
          </div>
        )}
...
export default MarketplaceTermsDocument;
