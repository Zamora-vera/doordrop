        ...
        if (paidOrder.activated || ['paid', 'preparing'].includes(String(paidOrder.order?.status || ''))) {
          ...
          if (paid) {
            await prepareMarketplaceOrderShipment(String(paid.id)).catch((error: any) => {
              console.error('[Marketplace] Preparaci�n despu�s de PayPal pendiente:', error?.message || error);
            });
...
async function refreshMarketplaceCarrierQuote(order: any, packageData: any) {
  ...
  if (!quoteRows?.[0]) throw new Error('La cotizaci�n real no pudo guardarse.');
  return quoteRows[0];
}
...
async function prepareMarketplaceOrderShipment(orderId: string, packageData?: any) {
  ...
  if (String(order.status) === 'pending_payment') return { success: false, pendingPayment: true, message: 'El pago a�n no est� confirmado.' };
  if (!packageData && !order.package_confirmed_at) {
    await pool.query(`UPDATE marketplace_orders SET status = 'paid', shipping_block_reason = 'Pendiente de confirmaci�n del peso y medidas reales', updated_at = NOW() WHERE id = ? AND status = 'paid'`, [orderId]);
  ...
  if (![measured.weightKg, measured.lengthCm, measured.widthCm, measured.heightCm].every((value: any) => Number.isFinite(Number(value)) && Number(value) > 0)) {
    throw new Error('Peso y medidas reales no v�lidos.');
  }
  ...
  if (!['parcelabc', 'genei', 'paccofacile', 'spedirepro', 'spediamopro', 'easypost', 'logihub_intl'].includes(providerCode)) {
    throw new Error('La cotizaci�n no pertenece a un proveedor log�stico real habilitado.');
  }
  ...
  if (!shipmentId) {
    ...
    await TrackingEventRepo.create({ shipment_id: shipmentId, tracking_code: tracking, status: 'pending_customer_balance', status_label: 'Pendiente de saldo', description: 'El env�o Marketplace espera saldo del vendedor para comprar la etiqueta.' }).catch(() => null);
  ...
  return { success: Boolean(result?.success), shipmentId, pendingLabel: !fresh?.label_url && !fresh?.label_base64, pendingBalance: fresh?.status === 'pending_customer_balance', trackingCode: fresh?.tracking_code || null, labelUrl: fresh?.label_url || null, message: result?.message || 'Env�o en preparaci�n.' };
}
...
async function requestMarketplaceSellerPayout(orderId: string, sellerId: string) {
  ...
  if (!['protection_period', 'delivered', 'completed'].includes(String(order.status))) throw new Error('El retiro solo puede solicitarse despu�s de la entrega.');
  ...
  if (!amountMinor) throw new Error('El importe disponible para retiro no es v�lido.');
  ...
  return { success: true, payout: payoutRows?.[0], message: `Solicitud creada. El retiro estar� disponible despu�s de la entrega m�s 14 d�as (${eligibleAt.toISOString().slice(0, 10)}).` };
}
  ...
  for (const payout of rows) {
    ...
    try {
      ...
      const response = await fetch(`${ship24goPayPalApiBase(keys?.paypalEnvironment)}/v1/payments/payouts`, {
        ...
        body: JSON.stringify({ sender_batch_header: { sender_batch_id: batchId, email_subject: 'Tu retiro DoorDrop est� en proceso', email_message: 'Tu saldo Marketplace fue liberado despu�s del periodo de protecci�n.' }, items: [{ recipient_type: 'EMAIL', amount: { value: (Number(payout.amount_minor) / 100).toFixed(2), currency: payout.currency || 'EUR' }, receiver: payout.paypal_email, note: `DoorDrop Marketplace ${payout.order_number}`, sender_item_id: payout.id }] })
      });
      const data: any = await response.json().catch(() => ({}));
      if (!response.ok || !data?.batch_header?.payout_batch_id) throw new Error(data?.message || 'PayPal no acept� el retiro.');
